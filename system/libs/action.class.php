...
class Action 
{
	...
	private static function setMessageToStack($stack, $message, $module = "CMS") 
	{
		...
		if  (defined('AJAX') && AJAX) {
            if (defined('CMS_PHPCONSOLE') && CMS_PHPCONSOLE && $connector->isActiveClient()) {
            
            } else {
                if (empty($_RESULT['action_error'])) {
                	$_RESULT['action_error'] = "";
                }
                $_RESULT['action_error'] = $message;
            }
	...
	public static function finish($error = false) 
	{
		...
		//$_REQUEST['_return_path'] = urldecode($_REQUEST['_return_path']);
	...
	public static function displayLog() 
	{
		...
		if ( IS_DEVELOPER ) $TmplDesign->set('show_responce_log', true);	
	 ...
	 * @return void
	 */ 
	public static function saveExtendedLog($source='admin') 
	{
	...
	/**
	 * Сохраняет расширенный лог изменений
	 * 
	 * @return void
	 */ 
	static function saveEventLog($source='admin')
	{
		if (!isset($_REQUEST['_event'])) return;
               
		/**
		 * Если внизу таблицы нажать на кнопку "сохранить изменения", то событие будет передано в виде массива с координатами
		 * которым был сделан щелчёк [_event][cms/table_update] => array(x,y)
		 */ 
		if (is_array($_REQUEST['_event'])) {
			$_REQUEST['_event'] = array_keys($_REQUEST['_event']);
			$_REQUEST['_event'] = reset($_REQUEST['_event']);
	...
	/**
	 * Сохраняет сообщение об успешном выполнении комманды, повторяющиеся сообщения не дублируются
	 * @param string $message
	 */
	public static function setOk($message,$destination_url="") 
	{
		global $_RESULT;

		if (defined('AJAX') && AJAX) {
            $_RESULT['action_success'] = (isset($_RESULT['action_success'])) ? $message+';'.$_RESULT['action_success'] : $message;
            if ( $destination_url ) {
                $_RESULT["javascript"]  = globalVar($_RESULT["javascript"], "");
                $_RESULT["javascript"] .= "location.href='$destination_url'";
            }

		} else {
            $_SESSION['ActionReturn']['ok'][md5($message)] = $message;
		}
	}
