 ...
 * @return array
 */
function parse_headers($name, $headline, $title, $description, $addditional='') 
{
    $result['name'] = $name;
	$result['headline'] = empty($headline) ? $name : $headline;
	$result['title'] = empty($title) ? $result['headline'] : $title;
	$result['description'] = empty($description) ? $result['title'] : $description;
	
	// Удаляем теги с мета тегов и заголовка страницы	
    if(!empty($addditional)){
        reset($result);
        while(list($key,$value)=each($result)){
            $result[$key] = str_replace('&nbsp;', '', strip_tags($value));
            $result[$key] .= (preg_match('/\.$/', $result[$key])) ? $addditional : ".".$addditional;                 
        }
    }
	return $result;
}
...
/**
 * Добавляет значение в строку запроса, при этом удаля старое значение этого параметра.
...
/**
 * Массив доступных языков для сайта
 * @return array 
 */
 ...
 * @return array 
 */
function getLanguageActiveList()
{
    $languages = array('ru', 'ua', 'en');
    return $languages;
}
/**
...
* 
* @param mixed
* @param boolean - выводить для не администраторов сайта
* @param boolean - остановка обработки
* @return void
*/
function x($array, $show = false, $end = false) 
{
	if (!defined('ERROR_OCCUR')) {
        define('ERROR_OCCUR', 1);
    }
    
	if (!IS_DEVELOPER && !$show) {
        return;
    }
	...
	if (defined('STDIN')) {
        ...
        echo '<PRE style="text-align:left;font-size:12px;background-color: #f0f0f0; padding: 10px; margin-top: 20px; margin-bottom: 20px;">'
        . '     <b><font style="color:red;">Вывод дампа:</font></b><br>'.$debug_backtrace[0]['file'] . ' ('. $debug_backtrace[0]['line'] . ')<BR/>';
        print_r($array);
        echo '</PRE>';
	} 
...
function x_debug($array, $show = false)
{
    if (!defined('ERROR_OCCUR')) {
        define('ERROR_OCCUR', 1);
    }
    ...
    if ( defined('CMS_PHPCONSOLE') && CMS_PHPCONSOLE  ){
        global $connector;
        $message_output = new DebugMessage();
        $message_output->tags = array("Debug Info");
        $message_output->data = print_r($array,true); 
        $message_output->file = $debug_backtrace[0]['file'];
        $message_output->line = $debug_backtrace[0]['line'];         
        $connector->sendMessage($message_output);
...
function x_error($message)
{
    ...
    if ( defined('CMS_PHPCONSOLE') && CMS_PHPCONSOLE ){
        global $connector;
        $message_output = new ErrorMessage();
        $message_output->class = 'CMS_ERROR';
        $message_output->data = $message;
        $message_output->code = E_WARNING;
        $message_output->file = $debug_backtrace[0]['file'];
        $message_output->line = $debug_backtrace[0]['line'];
        $message_output->trace = array();        
        $connector->sendMessage($message_output);
        unset($message_output);
    }    
}
...
{        
    if (CMS_COMPRESSION) {
         //if COMPRESS == 1 css and js to gzip   
        if (COMPRESS && CMS_INTERFACE == 'SITE' && !isset($_GET['nocache'])) {
            
            $content = MinifyHTML::minify($content, array(
                'cssMinifier' => array('MinifyCSS', 'minify'),
                /*'jsMinifier' => array('JSMin', 'minify'),*/
                'development' => COMPRESS
            ));        
            
        }
        
        if (isset($_GET['nocache'])) {
            $caches = array(CACHE_ROOT . 'css/', CACHE_ROOT . 'js/');
	...
	if (CMS_COMPRESSION && isset($_SERVER['HTTP_ACCEPT_ENCODING']) && stripos($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip')!==FALSE) {
...
function id2url($content, $editor = false) 
{
	...
	while (list($id, $url) = each($id2url)) {
		$pos = strpos($url, '/');
		$content = str_replace('{url:'.$id.'}', 'https://'.substr($url, 0, $pos+1).LANGUAGE_URL.substr($url, $pos+1), $content);
		unset($all_id[$id]);
	}
...
function errorHandler($error_number, $message, $file, $line) {
	...
	while(list($index,) = each($backtrace)) {
		if (isset($backtrace[$index]['file'])) {
			$debug[] = delta_path( $backtrace[$index]['file'].' ('.$backtrace[$index]['line'].')' );
		}
	}
	
    if (!isset($error_name[$error_number])) { 
        $error_name[$error_number] = '/'.$error_number.'/';
    }
	...
	if (defined('STDIN')) {
        ...
        if ( defined('CMS_PHPCONSOLE_ERROR') && CMS_PHPCONSOLE_ERROR){
            global $connector;
            $message_output = new ErrorMessage();
            $message_output->class = $error_name[$error_number];
            $message_output->data = $message.'<br>'.implode(' =><br>', $debug);
            $message_output->code = $error_number;
            $message_output->file = substr($file, strlen(SITE_ROOT));
            $message_output->line = $line;
            $message_output->trace = array();        
            $connector->sendMessage($message_output);
            unset($message_output);   
        } 
    ...
    $host = defined('CMS_HOST') ? CMS_HOST : '';
    $http_ip = defined('HTTP_IP') ? HTTP_IP : '';
    $http_locale_ip = defined('HTTP_LOCAL_IP') ? HTTP_LOCAL_IP : '';
	$data = array(
			'site_name' => $host,
			'cms_version' => (defined('CMS_VERSION')) ? CMS_VERSION: '',
			'date' => date('Y-m-d H:i:s'), 
			'url' => (defined('CURRENT_URL_FORM') ? "//".$host.CURRENT_URL_FORM : 'shell'),
			'ip' =>  $http_ip." (".$http_locale_ip.")",
	...
	if (is_writable(LOGS_ROOT.'error.log') && filesize(LOGS_ROOT.'error.log')/(1000 * 1000) < 100) {
		...
		fwrite($fp, "\n
                ...
                Date: ".date('Y-m-d H:i:s')."
                URL: ".(defined('CURRENT_URL_FORM') ? "//".$host.CURRENT_URL_FORM : 'shell')."
                IP: ".(defined('HTTP_IP') ? HTTP_IP : '') ."
                File: ".substr($file, strlen(SITE_ROOT))."
                Mtime: ".(isset($stat['mtime']) ? $stat['mtime'] : ' no defined stat') . "
                Line: ".$line."
                Type: ".$error_name[$error_number]."
...
{    
    //$max_length = ( $max_length > 40 ) ? 40 : $max_length;
...
/**
 * Преоброзование числа
 *   12030 => 12 030, 1245,2333 => 1 245.23
 * @param decimal $price
 * @return type
 */
function num2str($number)
{
    ...
    $number = (round($number, 2) == round($number)) ?  number_format($number, 0, ',', " ") : number_format($number, 2, ',', " ");    
    return $number;    
}

/**
 * Преоброзование числа для записи в Базу данных
 *   12030 => 12030, 1245,2333 => 1245.23
 * @param decimal $price
 * @return type
 */
function num2db($number)
{
    if (empty($number)) {
        return 0;
    }
...
function html_screen($path, $content, $action='', $stage='')
{
	...
	if(!is_dir($logs_dir_local)) {
        mkdir($logs_dir_local, 0777, true);
    }
	...
	if (rand(0, 1000) > 900) {
		...
		while (list(, $date) = each($logs)) {
			...
			if($tstamp < time()-86400*3) {
                Filesystem::delete(LOGS_ROOT.$path.$date);  
            }
		}
	} 
}
...
/**
 * Возращает одно из слов массива в соответствии с числом $digit
 * 
 * @param int $n - Количество
 * @param array $titles - (товар, товара, товаров )
 * @return string
 */
function word_completion($n, $titles) 
{
...
/*
 * path dirname(windows+)
 */
function delta_path($param) 
{
    if (substr(PHP_OS, 0, 3) == 'WIN') {
        $param = str_replace('/\\', '/', $param);
        $param = str_replace('\\', '/', $param);
...
{
    $text = strip_tags($text, '<p><h3><h4><h5><table><td><th><tr><b><strong><i><u><del><strike><em><center><h2><li><ol><ul><br><hr>');     
    $text = preg_replace('/<([a-z][a-z0-9]*)[^>]*?(\/?)>/i','<$1$2>', $text); 
    $text = preg_replace("/(data-scayt_word=\".+?\"|data-scaytid=\".+?\")/", "", $text); 
...
/**
 * Розбивает строку на массив, разделители: запятая, пробел или перевод строки
 * 
 * @param string $string
 * @return array
 */
function dexplode($text)
{
    return preg_split("/[\s\n\r\t,]+/", $text, -1, PREG_SPLIT_NO_EMPTY);
}

/**
 * Преоброзование числа в строку
 * @param decimal $num
 * @param string $course
 * @return type
 */
function num2strnum($num, $course = 'грн')
{
    $nul = 'нуль';
    $ten = array(
    ...
    $hundred = array('','сто','двісти','триста','чотириста','п`ятсот','шістсот', 'сімсот','вісімсот','дев`ятсот');
    $unit = array( // Units
        array('коп.' ,'коп.' ,'коп.', 1),
        array( $course.'.' ,$course.'.' ,$course.'.', 0),
    ...
    $nums = explode(',', sprintf("%015.2f", $num));    
    $uah = $nums[0];
    $kop = (isset($nums[1])) ? $nums[1] : 0;
    ...
    $out[] = morph(intval($uah), $unit[1][0],$unit[1][1],$unit[1][2]); // uah
    $out[] = str_pad($kop, 2, '0', STR_PAD_LEFT) . ' ' . morph($kop,$unit[0][0],$unit[0][1],$unit[0][2]); // kop
 ...
 * @return type
 */
function mb_ucfirst_up($str) {
    return mb_strtoupper(mb_substr($str, 0, 2), 'utf-8')  . mb_substr($str, 2);
}

function to_two_titles($title) {
   
    $data = array();
    
    $titles = explode(' ', $title);
    $d = floor(count($titles) / 2);
    if ($d == 0) {
        $data['title_1'] = $title;
    } else {
        $s = array_chunk($titles, $d, false);
        $data['title_1'] = implode(' ', $s[0]);
        $data['title_2'] = implode(' ', $s[1]) . (isset($s[2]) ? ' ' . implode(' ', $s[2]) : '');
    }
    return $data;
}
