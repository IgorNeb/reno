...
class TemplateDB extends Template 
{
	...
	public function __construct($table_name, $module, $template, $language = null) 
    {
		...
		if (!isset(self::$modules[$module])) {
            trigger_error(cms_message('CMS', 'Модуль %s - не существует.', $module), E_USER_ERROR);
        }
		...
		if ($DB->rows == 0) {
            trigger_error(cms_message('CMS', 'Шаблон %s в модуле %s - не существует.', $template, $module), E_USER_ERROR);
        }
		...
		if (is_file($this->compiled_file)) {
			...
			if ($compiled_stat['mtime'] > $template_data['update_tstamp'] && $compiled_stat['mtime'] > $class_stat['mtime']) {
                $compile = false;
            }
		}
		...
		if ($compile) {
            $this->saveCompiledTemplate($this->compiled_file, $template_data['content']);
        }
	}
	...
	protected function initMailDesign($template)
    {
		...
		if(empty($template['table']) || $template['table'] != 'cms_mail_template' || empty($template['design_id'])){
