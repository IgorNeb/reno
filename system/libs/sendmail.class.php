...
class Sendmail
{
	 ...
	 */
	public function __construct($account_id, $subject, $content, $add_to_template = true)
    {
		global $DB;
        
        if (CMS_MAIL_DESIGN && $add_to_template) {
            $Template = new Template('cms/site/mail');
	...
	public function send($recipient, $immidiatley = false) 
    {
		...
		while (list(,$email) = each($recipient)) {
			...
			if ($immidiatley == true && !CMS_MAIL_UNISENDER) {
	...
	private function queue($headers, $body, $recipient) 
    {
		...
		$query = "
			INSERT INTO cms_mail_queue SET
				...
				subject='".$DB->escape($this->subject)."',
	...
	public static function delivery($queue_id = null, $mailserver = null)
    {
        ...
        if (CMS_MAIL_UNISENDER && MAILLIST_UNISENDER_KEY) {
                 
            $return = array('sender' => $mailserver['sender_email'], 'recipient' => $message['recipient'], 'delivery' => 'error', 'status_message'=>'');
		
            $unisender_mail_id = CMS_MAIL_UNISENDER_ID;
            $mailserver = $DB->query_row("select * from cms_mail_account where id='$unisender_mail_id'");
            
            $uniSender = new UnisenderApi(MAILLIST_UNISENDER_KEY);            
            $result = $uniSender->sendEmail(array(
                'email' => $message['recipient'],
                'sender_name' => $mailserver['sender_name'],
                'sender_email' => $mailserver['sender_email'],
            ...
            //x($answer);
            if (isset($answer->result->email_id)) {                
                $result = $uniSender->checkEmail(array(
                    'email_id' => $answer->result->email_id
                ));
                $deliveries = json_decode($result);
                
                $subquery = "unisender_emailid='{$answer->result->email_id}', ";
                
                $return['delivery'] = "sending";
                 
                if (isset($deliveries->result->statuses)) {
                    $delivery = $deliveries->result->statuses[0];
                    if (isset($delivery->status)) {
                        if ($delivery->status == 'not_sent') {                        
                            $return['delivery'] = $delivery->status;
                        } elseif (preg_match('/^ok_/', $delivery->status)) {
                            $return['delivery'] = 'ok';                        
                            $subquery .= "headers=null, body=null,";

                        } elseif (preg_match('/^err_/', $delivery->status)) {
                            $return['delivery'] = 'error';   
                        }                    

                        $return['status_message'] = $delivery->status;
                    }
                    
                } elseif (isset($deliveries->error)) {
                    $return['delivery'] = 'error';   
                    $return['status_message'] = $deliveries->error;
                }
                
            }  elseif (isset($answer->error)) {            
                $return['status_message'] = $answer->error;
            }
        } else {
