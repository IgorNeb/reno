...
class Shell 
{
	 ...
	 * @param string $flag
	 */
	public static function addQueue($flag) 
    {
	 ...
	 */
	public static function checkQueue($flag, $reload = false) 
    {
	...
	* @return string
	*/
	public static function exec_stdin($command, &$stdin, &$error)
    {
		$stdout = '';
	...
	* @return void
	*/
	public static function collision_catcher($cron='', $uniq_id = '', $max_execution_time = false) 
    {
        ...
        if ( $cron == "" ) $script_name = delta_path($debug['file']);
        else $script_name = $cron;
	 ...
	 */
	public static function dbLog($url, $start_date, $status = 'failed')
    { 
		global $DB, $page_status, $page_content, $output_messages;
        
        if ( $page_status && ($status != 'blocked') ) {
            $status = $page_status;
        }

        $page_content = $output_messages;
		...
		$DB->insert("
                    INSERT INTO cms_crontab_history 
                    SET crontab_id = '$id', start_dtime = '$start_date', end_dtime = '$end_date', status = '$status', 
                        message = '$page_content', owner = '$owner'
                ");
	}
	 ...
	 * @return string
	 */
	public static function getProcessCmdline($pid) 
    {
	 ...
	 */
	public static function waitSemaphore($short_name) 
    {
	 ...
	 * @return bool
	 */
	public static function isProcessRunning($pid, $command_part = '')
    {
	 ...
	 * @return bool
	 */
	public static function runCommandWithTimeout($command, $timeout, &$output)
    {
	...
	public static function getProcessLimits($pid) 
    {
		$result = array(
