...
class cmsShowEdit
{
	...
	*/
	...
	public function __construct($table_id, $id_list, $copy = false) 
    {
		...
		$this->Template->setGlobal('title', $this->table['title']);
	...
	public function parseFields() 
    {
		...
		while (list($field_name, $field) = each($this->fields)) {
			...
			// Игнорируем поля, к которым стоит условие фильтра, и оно не исполняется
			if (!empty($field['select_condition'])) {                             
                $is_continue = $this->checkCondition($this->data, $field['select_condition']);
                if ($is_continue) {
                    continue;                    
                }
			}
			...
			} elseif ($field['cms_type'] == 'color') {
				
				$this->showColor($field_name);
...
//				trigger_error(cms_message('CMS', "Невозможно определить формат вывода поля %s. Более подробную информацию смотрите в описании таблицы `%s`.", $field_name, $this->table['name']), E_USER_ERROR);
    ...
    /**
     * Если условие $string не удовлетворено, тогда данное поле не выводится для редактирования  
     *      Условие добавляется при описания полей таблицы
     * @param array $row - данные текущей таблицы
     * @param string $string - условие для отбора
     * @return boolean 
     */
    public function checkCondition($row, $string) 
    { 
        $is_condition = true;             
        $string = " if( ".$string." ) { \$is_condition = false; }   ";
        eval($string);
        return $is_condition;
    } 
	...
	*/
    ...
    /**
	 * Создает элемент формы color
	 * @param string $field_name
	 * @return void
	 */
	private function showColor($field_name) 
    {
		$this->Template->iterate('/devider/row/', $this->tmpl_devider, array(
				'type'=>'color', 
				'row' => $this->current_row_param, 
