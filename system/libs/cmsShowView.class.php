...
class cmsShowView 
{
	...
	private $param = array(
		...
		'show_class_disabled' => true, //отображать строку таблицы в стиле disabled
		...
		'excel' => false, // Отображать ссылку для скачивания таблицы в формате Excel
		'help' => array(), // Отображать Справочник (cms_documentation)
		'priority' => true, // Отображать колонку сортировки данных
        '_id' => true, // отображать ли кнопки редактирования
	);
	...
	*/
	public function display()
    {
		...
		if (IS_DEVELOPER) {
            if ($this->SQLParser->tables > 1) {
                $table_content = ' Вывод таблиц: ';
            } else {
                $table_content = ' Вывод таблицы: ';
            }

            $tables_content[] = "<a href='/admin/cms/db/tables/fields/?table_id=".$this->table['id']."'>[".$this->table['name']."]</a>";
            reset($this->SQLParser->tables);
            while (list($table_name,$table_key)=each($this->SQLParser->tables)) {
                $table_info     = cmsTable::getInfoByAlias($this->DBServer->db_alias, $table_key);
                if (!count($table_info) || $table_info['id'] == $this->table['id']) {
                    continue;
                }
                $tables_content[] = " <a href='/admin/cms/db/tables/fields/?table_id=".$table_info['id']."'>[".$table_info['name']."]</a>";
            }
            $table_content .= implode(", ", $tables_content);
            $this->Template->set('table_title_table', $table_content . '<br>');
		} 
		...
		$this->Template->set('parent_link', $this->param['parent_link']);
         ...
         //Добавляем справочник к отображению таблицы
        if (!empty($this->param['help'])) {
            $help = cmsDocs::getByIds($this->param['help']);
            $this->Template->set('table_help', $help);
        }
		...
		if ($this->SQLParser->total_rows > 0 && isset($_GET['output'][self::$instance_number]) && $_GET['output'][self::$instance_number] == 'xls') {
			$this->outputXls();
		}
	...
	private function createExtraColumns()
    {
		...
		if (isset($this->fields['priority']) && $this->param['priority']) {
			$this->addColumn('priority', '5%', 'center', cms_message('CMS', 'Порядок'), '<a class = "sort"></a><input type="hidden" name="id[]" value="{id}">');			
		}
		
		// Колонка с ссылкой на редактирование
		if ($this->param['edit']) {
			$this->addColumn('edit', '5%', 'center', cms_message('CMS', 'Ред.'), '<a href="javascript:void(0);" class="edit" onclick="EditWindow(\'{id}\', '.$this->table['id'].', \''.CMS_STRUCTURE_URL.'\', \''.CURRENT_URL_LINK.'\', \''.LANGUAGE_CURRENT.'\', \'\');return false;" title="'.cms_message('CMS', 'Редактировать').'"></a>');
		}
		
		// Колонка с сылкой на удаление
		if ($this->param['delete']) {
			$this->addColumn('del', '5%', 'center', cms_message('CMS', 'Удл.'), '<a href="/action/admin/cms/table_delete/?_return_path='.CURRENT_URL_LINK.'&_language='.LANGUAGE_CURRENT.'&_table_id='.$this->table['id'].'&'.$this->table['id'].'[id][]={id}&_language_'.LANGUAGE_CURRENT.'" class="del" title="'.cms_message('CMS', 'Удалить').'" onclick="return confirm(\''.cms_message('CMS', 'Удалить').'?\')"></a>');
		}
	}
	...
	*/
	private function makeRowTemplate()
    {
		...
		$result = '<tr class="{_class}" id=tr'.self::$instance_number.'_{id} >';
		reset($this->columns_priority);
		while (list(, $field_name) = each($this->columns_priority)) {
			...
			if ($val['editable']) {
				...
				if ($pilot_type == 'boolean') {
					...
					$val['text'] = '
						...
						<input class="delta-checkbox" type="checkbox" name="'.$this->table['id'].'[{id}]['.$val['name'].']" value="1" {_'.$val['name'].'_checked}>
					';
	...
	*/
    ...
    {        
		...
		if (isset($row['id']) && $this->inserted_id == $row['id']) {
            ...
            $row['_class'] = ($this->row_index % 2) ? 'odd' : 'even';
            
            if (!empty($this->param['class_field']) && isset($row[$this->param['class_field']]) && !empty($row[$this->param['class_field']])) {
                $row['_class'] .= ' ' . $row[ $this->param['class_field'] ];
            } 
            
            if ($this->param['show_class_disabled'] && isset($row['active']) && $row['active'] == 0) {
                $row['_class'] .= ' disabled';
            }
        }
    ...
    {	
		// Checkbox column
		if ($this->param['delete'] || $this->param['edit'] || $this->param['_id']) {
			// Исправление от 18.12.2008 назване колонки поменяли с id на _id, так как не было возможности выводить колонку id
			$this->addColumn('_id', '2%', 'center', '<input type="checkbox" class="check_all delta-checkbox">', '<input type="checkbox" class="delta-checkbox delta-id" name="'.$this->table['id'].'[id][]" value="{id}" class="id">', true);
		}
    ...
    private function outputXls()
    {
		...
		require_once (SITE_ROOT."system/libs/excel_classes/PHPExcel/IOFactory.php");
		ob_end_clean();
		
		/**
		 * Имя листа обязательно должно быть указано и содержать не более 30 символов
		 */
		$title = (empty($this->param['title']) ? cms_message('cms', 'Таблица') : substr($this->param['title'], 0, 50));
		...
		unset($columns['priority'], $columns['priority_text'], $columns['edit'], $columns['del'], $columns['up'], $columns['down'], $columns['_id']);
		 ...
		 * 100% ширины таблицы
		 */
		$page_width = globalVar($_GET['output_xls_width'], 150);
		...
		while (list($index,$width) = each($column_width)) { 
			$objPHPExcel->getActiveSheet()->getColumnDimension($alpha[$index])->setWidth($width);
		}
        ...
        $objPHPExcel->getActiveSheet()->mergeCells('A1:'.$alpha[$last].'1');

        $objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->getColor()->setARGB( 'ff1e90ff' );
        $objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setBold(true)
                                      ->getColor()->setARGB( 'ff00bfff' );
        ...
        $head_row = $alpha[0].'2:'.$alpha[$last].'2';
        $objPHPExcel->getActiveSheet()->getStyle( $head_row )->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
                                      ->getStartColor()->setARGB('FFA9A9A9');
        $objPHPExcel->getActiveSheet()->getStyle( $head_row )->getFont()->setBold(true)
                                      ->getColor()->setARGB(PHPExcel_Style_Color::COLOR_WHITE);
        $objPHPExcel->getActiveSheet()->getStyle( $head_row )->getAlignment()
                                      ->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER)
                                      ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
                                      ->setWrapText(true);	
        $c1 = 2; $c2 = 3;
		...
		if (count($this->merge_columns)==0) {
			...
			while (list($index, $row) = each($columns)) { 
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alpha[$counter].$c1, $row['title']);
			...
			while (list($index,$row) = each($columns)) {
				...
				if (!isset($this->merge_columns[$row['name']])) {
					 ...
					 */
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alpha[$counter].$c1, $row['title']); 
                    $objPHPExcel->getActiveSheet()->mergeCells($alpha[$counter].$c1.':'.$alpha[$counter].$c2);
				} else {
					/**
					...
					if (!isset($this->merge_title[$merge_title]['printed'])) {
                        ...
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alpha[$counter].$c1, $this->merge_columns[$row['name']]['title']); 
						$counter_c2 = $counter + $this->merge_columns[$row['name']]['count']-1;
                        $objPHPExcel->getActiveSheet()->mergeCells($alpha[$counter].$c1.':'.$alpha[$counter_c2].$c1);
						$this->merge_title[$merge_title]['printed'] = true;
					}
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alpha[$counter].$c2, $row['title']); 					
				}
			...
			while (list(, $column) = each($columns)) { 
				...
				if (isset($row[$column['name']])) {
					...
					if (
                        ...
                        $counter_c2 = $data_start_row+$row_counter;
                        $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alpha[0].$counter_c2, $row[ $this->param['subtitle'] ] ); 
                        $objPHPExcel->getActiveSheet()->mergeCells($alpha[0].$counter_c2.':'.$alpha[$last].$counter_c2);

                        $head_row = $alpha[0].$counter_c2.':'.$alpha[$last].$counter_c2;
                        $objPHPExcel->getActiveSheet()->getRowDimension($counter_c2)->setRowHeight(21);
                        $objPHPExcel->getActiveSheet()->getStyle( $head_row )->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
                    ...
                    if($column['name'] == 'active'){
                        $value = ( $value == 1 ) ? cms_message('cms', 'Да') : cms_message('cms', 'Нет');
                    }
					...
					if ($column['align'] == 'right') {
						$format = PHPExcel_Style_Alignment::HORIZONTAL_RIGHT;
					} elseif ($column['align'] == 'center') {
						$format = PHPExcel_Style_Alignment::HORIZONTAL_CENTER;
					} else {
						$format = PHPExcel_Style_Alignment::HORIZONTAL_LEFT;
					}
                    $counter_c2 = $data_start_row + $row_counter;
                    $objPHPExcel->setActiveSheetIndex(0)->setCellValue($alpha[ $column['index']].$counter_c2, $value); 
                    $objPHPExcel->getActiveSheet()->getStyle($alpha[ $column['index']].$counter_c2)->getAlignment()->setHorizontal($format);
                    $objPHPExcel->getActiveSheet()->getRowDimension($counter_c2)->setRowHeight(17);                    
				} 
			}
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A2:'.$alpha[$last].$counter_c2)->applyFromArray($styleArray);
        ...
        $filename = (isset($_SESSION['xlsx_filename'])) ? $_SESSION['xlsx_filename'] : $this->table['table_name'];
    ...
    /**
     * Формирование верхнего меню для страниц
     * 
     * @param array $data
     * @param text $content
     * @return text
     */
    public static function topMenu($data, $content, $class = '')
    {
        $Template =  new Template('cms/admin/top_menu');
        $Template->iterateArray('/row/', null, $data);
        $Template->set('content', $content);
        $Template->set('class', $class);        
        return $Template->display();
    }
    
    public static function modifTopMenu($modif_id, $structure_id, $content)
    {
        global $DB;
        
        $modif = $DB->query_row("SELECT CONCAT('/model/', tb_modif.uniq_name, '/') as url, tb_modif.active, tb_modif.active_admin "
            . " FROM auto_modification tb_modif "
            . " INNER JOIN `auto_model` AS tb_model ON tb_model.id = tb_modif.model_id AND tb_model.active = 1 "
            . " WHERE tb_modif.id='$modif_id' ");
        
        $last =  (empty($modif) || ($modif['active'] == 0 && $modif['active_admin'] == 0 ))
                ? array('name'=>'Выключен', 'url' => '#', 'attribute' => "onclick='return false;' class='noactive'") 
                : array('name'=>'<i class="aicons inline views">&nbsp;</i>На сайт '
                    . (($modif['active_admin'] == 1 && $modif['active'] == 0) ? " (режим тестирования)" : ""), 
                    'url' => $modif['url'], 'attribute' => "target='_blank'");
                
        return cmsShowView::topMenu(array(
            array('name'=>'К модификациям', 'up' => true, 'url'=>'/admin/auto/cars/?modification_id=' . $modif_id),
            array('name'=>'<i class="aicons inline edit">&nbsp;</i> Редактировать', 'attribute' => "onclick=\"EditWindow('$modif_id', 2965, 'auto/cars/', '".$_SERVER['REQUEST_URI']."', 'ru', '');return false;\"", 'url'=>'javascript:void(0);'),
            array('name'=>'Инфоблоки', 'active' => ($structure_id == 2339), 'url'=>'/admin/auto/cars/infoblocks/?modification_id='.$modif_id),
            array('name'=>'Фото и видео', 'active' => ($structure_id == 2335), 'url'=>'/admin/auto/cars/media/?modification_id='.$modif_id),
            array('name'=>'Характеристики', 'active' => ($structure_id == 2336), 'url'=>'/admin/auto/cars/options/?modification_id='.$modif_id),
            array('name'=>'Аксессуары', 'active' => ($structure_id == 2338), 'url'=>'/admin/auto/cars/accessories/?modification_id='.$modif_id),
            array('name'=>'Авто в наличии', 'active' => ($structure_id == 2337), 'url'=>'/admin/auto/cars/carstock/?modification_id='.$modif_id),
            $last
        ), $content, 'modifMenu');
    }
}
