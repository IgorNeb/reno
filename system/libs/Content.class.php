	...
	*/
	public function save() {
        ...
        if (!empty($this->content)) {
            //Добавление класса для инфоблоков в тексте редактора
            preg_match('/infoblock/i', $this->content, $mathces);
            if (!empty($mathces)) {
                $this->content = str_replace('/(class=[\'\"]block_inside[\'\"])/i', "", $this->content);
                $this->content = preg_replace('/(<div>\{infoblock)/i', '<div class="block_inside">{infoblock', $this->content, -1);
            }
        }
        //Добавление врапера для текста редактора
        preg_match('/^(<div[\s]+class=[\'\"]conContent[\'\"][^>]*?>)/i', $this->content, $mathces);
        if (!empty($mathces)) {
            $this->content = preg_replace('/^(<div[\s]+class=[\'\"]conContent[\'\"][^>]*?>)/i', "", $this->content, -1);
            $this->content = preg_replace('/(<\/div>)$/', "", $this->content, -1);
        }
        if (!empty($this->content)) $this->content = '<div class="conContent">'.$this->content."</div>";
