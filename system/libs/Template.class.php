 ...
 *   @4.0 Добавлены ключи для циклов 
 *      $index - текущая итериция, начиная с 1
 *      $first - первая итерация
 *      $last  - последняя итерация
 *      @{section_subsection}__total - общее количество элементов
 * 
 * @package DeltaCMS
 * @subpackage CMS
 * @version 4.0
...
class Template 
{
	...
	*/
	...
	public function __construct($filename, $language = null) 
    {
        ...
        if (substr($filename, 0, strlen(SITE_ROOT)) != SITE_ROOT) {
			...
			$this->template_file = TEMPLATE_ROOT."$filename.$language.tmpl";
		} else {
			$this->compiled_file = CACHE_ROOT."template/".substr($filename, strlen(SITE_ROOT)).".$language.php";
			$this->template_file = $filename.".$language.tmpl";
		}
		...
		if (!is_file($this->template_file)) {
			trigger_error(cms_message('CMS', 'Файл с шаблоном %s - не существует.', $this->template_file), E_USER_ERROR);
		}
		...
		if (is_file($this->compiled_file)) {
			...
			$template_stat = stat($this->template_file);
		...
		if ($compile) {
			$this->saveCompiledTemplate($this->compiled_file, file_get_contents($this->template_file));
		}
	}
	 ...
	 * @return string
	 */
	public static function staticContentCallback($matches)
    {
	...
	*/
	public function iterate($section, $parent = 0, $data = array()) 
    {
        ...
        $key_section = '/'.implode('/', $section).'/';
		$parent = (is_null($parent)) ? 0 : $parent;
        
        if (isset($this->vars[$key_section]) && isset($this->vars[$key_section][$parent])) {
            $data['index'] = count($this->vars[$key_section][$parent]) + 1;            
            end($this->vars[$key_section][$parent]);  
            $key = key($this->vars[$key_section][$parent]);  
            $this->vars[$key_section][$parent][$key]['last'] = 0;       
        } else {
            $data['index'] = 1;
        }  
        if ($data['index'] == 1) {
        ...
        $data['last'] = 1;
		$this->vars[$key_section][$parent][$this->section_counter] = $data; 
        $this->setGlobalVar(implode('_', $section) . '__total', $data['index']);
	...
	public function iterateArray($section, $parent, $data) 
    {
		...
		while (list($index, $row) = each($data)) {
			$this->section_counter++;
            if (isset($this->vars[$key_section]) && isset($this->vars[$key_section][$parent])) {
                $row['index'] = $total = count($this->vars[$key_section][$parent]) + 1;
            } else {
                $row['index'] = $total = 1;
            }
            if ($row['index'] == 1) {
			...
			$this->vars[$key_section][$parent][$this->section_counter] = $row;
			$return[$index] = $this->section_counter;
		} 
        $this->vars[$key_section][$parent][$this->section_counter]['last'] = true;
        $this->setGlobalVar(implode('_', $section) . '__total', $total);
		return $return;
	}
	...
	*/
	public function display() 
    {
        ...
        if (DEBUG_PROOF && IP_ADMINS && CMS_INTERFACE == 'SITE') {
            echo '<!-- BEGIN TMPL: ' . str_replace(SITE_ROOT, '/', $this->template_file) . ' /-->';
        }        
		require($this->compiled_file);
        if (DEBUG_PROOF && IP_ADMINS && CMS_INTERFACE == 'SITE') {
            echo '<!-- END TMPL: ' . str_replace(SITE_ROOT, '/', $this->template_file) . ' /-->';
        }
