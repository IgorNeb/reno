	...
	public function buildFieldStatic() 
    {
		...
		$query = "
			insert into cms_field_static (
				...
				stick, show_in_filter, regexp_id, regular_expression, field_language,
				group_edit, priority, is_nullable, column_default, is_real, cms_type, is_multilanguage,
			...
			SELECT 
				...
				tb_field.regexp_id,
                                IFNULL(tb_regexp.regular_expression, '') as regular_expression,
				...
				case
					...
					when tb_field.field_type='color' then 'color'
	...
	public function checkTable($table_id)
    {
		...
		$query = "
			...
			where
				...
				and tb_field.table_id='$table[id]'
				and tb_field.name != 'id' and tb_field.name != 'uniq_name'
			order by tb_field.priority
		";
		$data = $DB->fetch_column($query);
		if ($DB->rows > 0) {
		...
		$query = "
			...
			where 
				table_id='$table[id]'
				and cms_type='error' and data_type <> 'timestamp'
			order by priority
		";
		$data = $DB->fetch_column($query);
		if ($DB->rows > 0) {
...
//		}
		...
		$query = "
			...
			where
				table_id='$table[id]'
				and is_obligatory=0 and name <> 'uniq_name'
