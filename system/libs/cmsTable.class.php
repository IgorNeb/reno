...
class cmsTable
{
	 ...
	 * @return int
	 */
	public static function getIdByName($db_alias, $table_name_id) 
    {
		global $DB;
	 ...
	 * @param int $table_id
	 */
	public static function getInfoById($table_id) 
    {
		global $DB;
	 ...
	 */
	public static function getInfoByAlias($db_alias, $table_name)
    {
		global $DB;
	 ...
	 * @return array
	 */
	public static function getFields($table_id) 
    {
		global $DB;
	 ...
	 * @return array
	 */
	public static function getEnumFields($table_name, $field = '') 
    {
		global $DB;
		...
		$query = "
                ...
                inner join cms_db as tb_db on tb_db.id=tb_table.db_id
                where tb_db.alias='default' and tb_table.name='$table_name'
        ";
        $data = $DB->query($query);
        ...
        return (!empty($field) && isset($enum[$field])) ? $enum[$field] : $enum;
	}
	 ...
	 * @return int
	 */
	public static function getParentTable($table_id) 
    {
		global $DB;
	 ...
	 * @return array
	 */
	public static function getParentTables($table_id) 
    {
		$return = array();
		do {
			$prev_table = $table_id;
	 ...
	 * @return array
	 */
	public static function getFkeyNNInfo($table_id, $fk_table_id, $fk_link_table_id)
    {
		global $DB;
	 ...
	 */
	public static function loadInfoList($fk_table_id) 
    {
	 ...
	 * @return array
	 */
	public static function loadInfoTree($fk_table_id) 
    {
		$fk_table = cmsTable::getInfoById($fk_table_id);
		$DB_fk = DB::factory($fk_table['db_alias']);
        
        $where  = ($fk_table_id == 2763) ? " AND table_id <> 2926 " : ""; //товары не выводим в выпадайке
        $where .= ($fk_table_id == 2918) ? " AND id > 1 " : ""; //trade in не выводим в общей выпадайке категорий товаров
        $where .= ($fk_table_id == 1017) ? " AND id <> 210 " : ""; //прошедшие не выводим в выпадайке
	 ...
	 * @return array
	 */
	public static function loadInfoCascade($fk_table_id)
    {
		global $DB;
	 ...
	 * @return string
	 */
	public static function showFK($fk_table_id, $id)
    {
	 ...
	 */
	public static function delete($db_alias, $table_name, $table_type = 'BASE TABLE') 
    {
		global $DB;
