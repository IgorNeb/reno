...
class Filesystem 
{
	...
	*/
	public static function getMaxFileId($dir)
    {
	 ...
	 * @param string $path
	 */
	public static function deleteEmptyDirs($path) 
    {
	 ...
	 * @return array
	 */
	public static function getDirContent($dir, $full_path, $show_dirs, $show_files, $final_slash = true) 
    {
     ...
     * @param bool $skip_logs
     * @return array
     */
	public static function getAllSubdirsContent($dir, $files_only, $skip_logs = false)
    {
		clearstatcache();
		...
		while (list($index, $current_dir) = each($dir)) {
			...
			*/ 
			...
			while (list(, $dir_file) = each($dir_files)) {
				if ($skip_logs && $dir_file == SITE_ROOT.'system/logs/') {
                    continue;
                }
                $dir_file = delta_path( $dir_file );
	 ...
	 * @return bool
	 */
	private static function copyDir($source, $destination, $replace)
    {
		clearstatcache();
	 ...
	 * @return bool
	 */
	public static function rename($source, $destination, $replace = false) {
		if (is_file($source)) {
	...
	* @return bool
	*/
	public static function copy($source, $destination, $replace = false) 
    {
	 ...
	 * @param string $source
	 * @param string $destination
     * @return bool Возвращает true, если каталоги и файлы в этих каталогах полностью идентичны
	 */
	public static function isEqualDirs($source, $destination)
    {
		$different = false;
