...
class cmsEditAdd 
{
	...
	public function __construct($table_id, $data, $update_form, $tmp_dir, $ajax_select) 
    {
		...
		while (list($field_name, $field) = each($this->fields)) {
            ...
            //Обрезаем большой текст для типа поля Tinytext
            if ($field['data_type'] == 'tinytext') {
                $this->NEW[$field_name] = Misc::word_wrapper($this->NEW[$field_name], 255, ''); 
            }
		}
	...
	private function buildNew($data)
    {
		...
		if (isset($this->fields['uniq_name']) && empty($data['uniq_name']) && isset($data[$this->table['fk_show_name']]) && !empty($data[$this->table['fk_show_name']])) {
			$data['uniq_name'] = name2url($data[$this->table['fk_show_name']], $this->fields['uniq_name']['max_length']);
                ...
                isset($data['uniq_name']) && ($this->table['name'] != 'seo_structure') 
                &&  (!isset($data[$this->table['parent_field_name']]) 
                        ||
                    (isset($data[$this->table['parent_field_name']]) && !empty($data[$this->table['parent_field_name']])) 
                    )
                ) {                    
                    $data['uniq_name'] = name2url($data['uniq_name'], $this->fields['uniq_name']['max_length']);
        }
	...
	public function dbChange()
    {
		...
		if (is_array($this->uploads)) {
			...
			while (list($field, $val) = each($this->uploads)) {
				...
				// Перемещаем закачанный файл и обрезаем большие изображения
        ...
        if (is_module('Seo') && SEO_ACTIVE && method_exists('Seo', $this->table['table_name'])) {
                    
            $table_name = $this->table['table_name'];
            $childs = Seo::getChilds($table_name, $this->NEW['id']);

            //Добавление || Обновление страниц в Seo структуре  
            Seo::update($table_name, $this->NEW['id']);

            if ($this->action_type == 'update' && isset($this->NEW['uniq_name'])) {
                if ($this->NEW['uniq_name'] != $this->OLD['uniq_name']){  
                    //обновление урла
                    Seo::updateUrl($table_name, $this->NEW['id'], $childs);

                } elseif (!empty($this->table['parent_field_name'])) {
                    //существует родительская таблица
                    $parent_field_name = $this->table['parent_field_name'];
                    
                    if ($this->NEW[$parent_field_name] != $this->OLD[$parent_field_name]) {  
                        Seo::changeParent($table_name, $this->NEW['id']);
                        Seo::updateUrl($table_name, $this->NEW['id'], $childs);                                    
                    }
                }    
            }
            ...
            //удаление редиректов
            Seo::checkLinkIsRedirect($table_name, $this->NEW['id']);        
		}
                 
        //удаление данных с кеша
        $Cache = new CacheSql($this->table['name']);
        $Cache->delete();
	...
	private function buidInsertQuery()
    {
		...
		while (list($field_name, $field) = each($this->fields)) {
			...
			if ($field_name == 'priority') {
                ...
                $value[$field_name] = "'".addslashes($this->NEW[$field_name])."'";
                $this->cvs[$field_name] = addslashes($this->NEW[$field_name]);
	...
	private function buildUpdateQuery() 
    {
		...
		while (list($field_name, $field) = each($this->fields)) {
			...
			if ($field['is_nullable'] && !empty($field['fk_table_id']) && empty($this->NEW[$field_name])) {
                ...
                $value[$field_name] = "`$field_name`='".addslashes($this->NEW[$field_name])."'";
				$this->cvs[$field_name] = addslashes($this->NEW[$field_name]);                                
			}
		}
		return $value;
	...
	private function getNextPriorityId()
    {
        ...
        if (isset($this->table['fk_order_direction']) && $this->table['fk_order_direction'] == "ASC") {
            ...
            $query = "UPDATE `".$this->table['db_name']."`.`".$this->table['name']."` SET priority = priority + 1 ";
