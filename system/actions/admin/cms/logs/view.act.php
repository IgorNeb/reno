...
while(!feof($file)){
   ...
   // preg_match('/Array[\s]*\((.*)\)[\s]*/ismU', $row['request'], $matched_request);
    ...
    //$row['session'] = (!empty($params['session'])) ? array_to_html(htmlspecialchars( print_r($params['session'], true) )) : array();
    
    $row['request'] = (!empty($params['request'])) ? '<pre>'.$row['request'].'</pre>' : array();
    $row['session'] = (!empty($params['session'])) ? '<pre>'.print_r($params['session'], true).'</pre>' : '<pre>'.print_r(array(), true).'</pre>';
