 ...
 */
if(strpos(strtolower($path), strtolower(LOGS_ACTIONS_ROOT)) === FALSE){
        Action::onError("Вы только что попали в черный список!");
	exit;
}
...
//$current_log  = (!empty($_SESSION['log_actions_current_log'])) ? $_SESSION['log_actions_current_log'] : '';
//if(!empty($current_log)) $_RESULT['javascript'] = "log_display('$current_log', 'date', 'desc', 0);";  
