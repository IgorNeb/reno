...
preg_match('/^(<div[\s]+class=[\'\"]conContent[\'\"][^>]*?>)/i', $content, $mathces);
if (!empty($mathces)) {
    $content = preg_replace('/^(<div[\s]+class=[\'\"]conContent[\'\"][^>]*?>)/i', "", $content, -1);
    $content = preg_replace('/(<\/div>)$/', "", $content, -1);
}
$content = str_replace('block_inside', "", $content);
$TmplDesign->set('content', id2url($content, true));
//echo $TmplDesign->display();die();
